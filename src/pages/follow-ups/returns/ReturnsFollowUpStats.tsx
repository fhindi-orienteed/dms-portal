...
interface ReturnsFollowUpStatsProps {
  followUps: ReturnsFollowUp[];
}

export default function ReturnsFollowUpStats({ followUps }: ReturnsFollowUpStatsProps) {
  ...
  const totalReturnAmount = followUps
    .filter(f => f.status === 'pending' || f.status === 'in_progress')
    .reduce((sum, f) => sum + f.returnAmount, 0);
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Return Value</p>
            <p className="text-2xl font-bold text-gray-900 dark:text-white">${totalReturnAmount.toFixed(2)}</p>
          </div>
        </div>
      </div>
    </div>
