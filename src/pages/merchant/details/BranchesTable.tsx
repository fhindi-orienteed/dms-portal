...
export default function BranchesTable({ branches, onAddBranch }: Props) {
  ...
  const columns = [
    {
      ...
      accessor: (branch: any) => <span className='font-medium text-gray-800 dark:text-white/90'>{branch.name}</span>,
    },
    {
      header: t('merchants.branches.address'),
      accessor: (branch: any) => <span className='text-gray-600 dark:text-gray-400'>{branch.address}</span>,
    },
    {
      header: t('merchants.branches.manager'),
      accessor: (branch: any) => <span className='text-gray-800 dark:text-white/90'>{branch.manager}</span>,
    },
  ];
