...
export default function UsersTable({ merchantId }: UsersTabProps) {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (user: any) => (
        <Badge color={user.role === 'Admin' ? 'primary' : 'success'}>{getTranslatedRole(user.role, t)}</Badge>
      ),
    },
    {
      header: t('merchants.users.email'),
      accessor: (user: any) => <span className='text-gray-600 dark:text-gray-400'>{user.email}</span>,
    },
    {
      header: t('merchants.users.status'),
      accessor: (user: any) => (
        <Badge color={user.status === 'Active' ? 'success' : 'error'}>{getTranslatedUserStatus(user.status, t)}</Badge>
      ),
    },
  ];
  ...
  return (
    <>
      ...
      />
      <AddUserModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onAdd={handleAdd} />
    </>
  );
}
